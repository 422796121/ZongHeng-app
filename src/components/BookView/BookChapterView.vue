<template>
	<div class="chapter-view" ref="chapt">
		<ul>
			<li>
				<book-header :topTitle="titleName" :detailid.sync="detailid" :where.sync="where"></book-header>
			</li>
			<li>
	...
	export default {
		...
		data() {
			return {
				sort: null, // 排序
				chapterArr: [], // 章节
				getId: this.$route.query.chapterid, // 书源ID
				order: [], // 顺序
				titleName: this.$route.query.topTitle, // 标题
				page: 1, // 页数,
				selectArr: [], // select数组
				selectCount: [], // select个数
				detailid: sessionStorage['detailid'],
				where: 'chapter'
			}
		},
		created() {
